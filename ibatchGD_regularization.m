function [theta,cost_history,counter,accu_plot] = ibatchGD_regularization(x,y,theta,alpha,isRaw,lambda,grid)
...
while flag
    ...
    current_cost = icomputeCost_regularization(x,y,theta,lambda,grid);
%        currentTest_cost = computeCost(x1,y1,theta);
%((past_cost-current_cost)<=0.01)) can be plugged in if below
    if (isnan(current_cost) || isinf(current_cost)|| (past_cost<current_cost) || (past_cost-current_cost)<0.01)  && isRaw==0
        flag = 0;
    elseif isRaw && (counter>=500) 
        flag=0;
    else
